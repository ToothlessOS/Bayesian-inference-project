...
def determineBurnIn(mu_samples_collection: list, tau_samples_collection: list,
                     mu_init_collection: list, tau_init_collection: list, fig_name: str):
    # Plot and save the convergence graph for mu
    _color_list = ['blue', 'green', 'red', 'cyan', 'magenta']
    plt.cla()
    for i in range(len(mu_samples_collection)):
        plt.plot(mu_samples_collection[i], 
                 color=_color_list[i % len(_color_list)], 
                 label=f"({mu_init_collection[i]:.2f},{tau_init_collection[i]:.2f})")
        
    plt.legend()
    plt.xlabel("Index")
    plt.ylabel("Value")    
    plt.savefig(f"data/{fig_name}_mu_convergence.png")

    # Plot and save the convergence graph for tau
    plt.cla()
    for j in range(len(tau_samples_collection)):
        plt.plot(tau_samples_collection[j], 
        color=_color_list[j % len(_color_list)], 
        label=f"({mu_init_collection[j]:.2f},{tau_init_collection[j]:.2f})")

    plt.legend()
    plt.xlabel("Index")
    plt.ylabel("Value")    
    plt.savefig(f"data/{fig_name}_tau_convergence.png")
